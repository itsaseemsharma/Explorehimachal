...
@touristdestination.route('/event', methods=['GET'])
def event():
    return render_template('event.html',loggedIn="aaa", firstname="sss", destinations="SSSs", cityname="S", type="a", descriptions="A")

@touristdestination.route('/event/event1', methods=['GET'])
def event1():
    print("welcome")
    return render_template('event1.html',loggedIn="aaa", firstname="sss", destinations="SSSs", cityname="S", type="a", descriptions="A")

@touristdestination.route('/event/event2', methods=['GET'])
def event2():
    print("welcome")
    return render_template('event2.html',loggedIn="aaa", firstname="sss", destinations="SSSs", cityname="S", type="a", descriptions="A")

@touristdestination.route('/event/event3', methods=['GET'])
def event3():
    print("welcome")
    return render_template('event3.html',loggedIn="aaa", firstname="sss", destinations="SSSs", cityname="S", type="a", descriptions="A")

@touristdestination.route('/event/event4', methods=['GET'])
def event4():
    print("welcome")
    return render_template('event4.html',loggedIn="aaa", firstname="sss", destinations="SSSs", cityname="S", type="a", descriptions="A")
...
# @app.route('/Shimla', methods=['GET'])
# def Shimla():
#     return send_file('TouristsDestination/templates/Shimla.html')

@touristdestination.route('/query', methods=['GET', 'POST'])
def query():
    print("welcome in query")
    if (request.method == "POST"):

        form_data = request.form
        user = form_data.get("userid")
        print("login failed")
...
@touristdestination.route('/submitForm', methods=['GET', 'POST'])
def submitForm():
    print("welcome in submitForm")
    if (request.method == "POST"):

        form_data = request.form
        print("login failed", form_data, dir(form_data))

        name = form_data.get("fname")
        email = form_data.get("email")
        phone = form_data.get("phone")
        person = form_data.get("person")
        trekname = form_data.get("trekname")
        message = form_data.get("message")
        print("entry sone in table contacts")
        service = Services(db)
        data = service.addcontacts(name, email, phone, person, trekname, message)
        return render_template('event1.html')

@touristdestination.route('/admin', methods=['GET', 'POST'])
def admin():
        service = Services(db)
        data = service.getcontacts()
        print("data = ", data)
        for entry in data:
            print("data entry =", entry.get("name"))
